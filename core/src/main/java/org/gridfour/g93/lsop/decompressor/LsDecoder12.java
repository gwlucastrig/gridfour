...
import org.gridfour.g93.HuffmanDecoder;
import org.gridfour.g93.IG93Decoder;
import org.gridfour.g93.PredictiveTransformType;
 ...
 */
strictfp public class LsDecoder12 implements IG93Decoder {
  ...
  private void unpackInitializers(byte[] packing, int seed, int nRows, int nColumns, int values[]) {
    ...
    // step 3, the second to last column ----------------------
    v = values[nColumns - 2];
    for (int i = 1; i < nRows; i++) {
      v += m32.decode();
      values[i * nColumns + nColumns - 2] = v;
    }
  ...
  public void analyze(int nRows, int nColumns, byte[] packing) throws IOException {
    if (codecStats == null) {
      ...
      codecStats[0] = new CodecStats(PredictiveTransformType.None);
      codecStats[1] = new CodecStats(PredictiveTransformType.None);
      codecStats[2] = new CodecStats(PredictiveTransformType.None);
      codecStats[3] = new CodecStats(PredictiveTransformType.None);
      codecStats[4] = new CodecStats(PredictiveTransformType.None);
